"""
Flatpak tab: remove unused Flatpak runtimes.
"""
...
class FlatpakTab(Gtk.Box):
    ...
    def _fill_list(self):
        ...
        if not self._entries:
            ...
            if not flatpak_available():
                ...
                msg = _("No unused Flatpak runtimes found")
            ...
            for entry in self._entries:
                ...
                self._checkboxes[entry.ref] = check
                row_box.pack_start(check, False, False, 0)

                row_box.pack_start(Gtk.Image.new_from_icon_name('package-x-generic', Gtk.IconSize.MENU), False, False, 0)
    ...
    def _on_remove_clicked(self, btn):
        selected = [ref for ref, check in self._checkboxes.items() if check.get_active()]
        if not selected:
            return
