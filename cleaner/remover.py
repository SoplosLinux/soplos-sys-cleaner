...
def delete_temp_paths(paths: list[str], progress_cb: Callable = None) -> tuple[bool, str]:
    """Remove temporary files and directories. Falls back to pkexec for permission-denied paths."""
    removed = 0
    errors = []
    need_root = []
    total = len(paths)
    ...
    for i, path in enumerate(paths):
        if progress_cb:
            progress_cb((i + 1) / total * 0.7, _("Removing {}...").format(os.path.basename(path)))
        try:
            ...
            need_root.append(path)
        except Exception as e:
            errors.append(_("Error in {}: {}").format(path, e))

    if need_root:
        if progress_cb:
            progress_cb(0.8, _("Preparing administrative tasks..."))
        quoted = " ".join(shlex.quote(p) for p in need_root)
        try:
            result = subprocess.run(
                ['pkexec', 'bash', '-c', f'rm -rf {quoted}'],
                capture_output=True, text=True
            )
            if result.returncode == 127:
                return False, _("Authentication cancelled.")
            elif result.returncode == 0:
                removed += len(need_root)
            else:
                errors.append(_("Error in {}: {}").format('pkexec', result.stderr.strip()))
        except Exception as e:
            errors.append(_("Unexpected error: {}").format(e))
...
def purge_locales_and_docs(paths: list[str], progress_cb: Callable = None) -> tuple[bool, str]:
    """
    Remove multiple directories (locales, help, docs) in a single pkexec session.
    """
    if not paths:
    ...
    full_shell_cmd = f"rm -rf {' '.join(safe_paths)}"
